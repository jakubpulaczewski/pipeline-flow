...
class TestUnitYamlParser:
    ...
    def test_parse_empty_yaml() -> None:
        with pytest.raises(ValueError, match="Either `yaml_text` or `file_path` must be provided"):
    ...
    @staticmethod
    def test_parse_yaml_file_success(temporary_yaml_file: str) -> None:
...
class TestUnitPluginParser:
    @pytest.fixture
    def mock_isdir(self: Self, mocker: MockerFixture) -> MockType:
    ...
    def test_get_all_files_with_directories_and_files(self: Self, mocker: MockerFixture) -> None:
...
class TestUnitPipelineParser:
    ...
    def test_create_pipeline_with_multiple_sources_destinations(self: Self, mocker: MockerFixture) -> None:
        pipeline_data = {
            "type": "ETL",
            "phases": {
                "extract": {
