...
# Project
from core.models import etl
from core.plugins import PluginConfig, PluginFactory, PluginLoader

T = TypeVar("T")


class FakeExtractPlugin(etl.IExtractor):
...
class FakeTransformPlugin(etl.ITransformer):
...
class FakeLoadPlugin(etl.ILoader):
...
class TestPluginFactory:
    ...
    def test_validate_invalid_etl_stage_plugin_interface(
        etl_stage: str, plugin_class: ETL_CALLABLE, mocker
    ) -> None:
        ...
        mock_base_class = mocker.patch.object(
            ETLConfig, "get_base_class", return_value=None
        )
    ...
    def test_validate_invalid_subclass_plugin_interface(mocker) -> None:
        ...
        mock_base_class = mocker.patch.object(
            ETLConfig, "get_base_class", return_value=etl.IExtractor
        )
    ...
    @staticmethod
    @pytest.mark.parametrize(
        "etl_stage, plugin_class, base_class",
        [
            ("extract", FakeExtractPlugin, etl.IExtractor),
            ("transform", FakeTransformPlugin, etl.ITransformer),
            ("load", FakeLoadPlugin, etl.ILoader),
        ],
    )
    def test_validate_plugin_interface_valid_etl_stage_and_subclass(
        etl_stage: str, plugin_class: ETL_CALLABLE, base_class: ETL_CALLABLE, mocker
    ) -> None:
        """Ensures that validation works for positive cases"""
        mock_base_class = mocker.patch.object(
            ETLConfig, "get_base_class", return_value=base_class
        )
...
class TestPluginConfig:
    ...
    def test_get_plugin_stage_mapper(
        etl_stage: str, expected_config: dict[str, list[str]]
    ) -> None:
...
class TestPluginLoader:
    ...
    def test_initialize_default_plugin(
        mock_validate_plugins, mock_plugin_stage_mapper, mock_importlib
    ) -> None:
        mock_plugin_stage_mapper.return_value = {"fake_engine": ["default_plugin1"]}
