...
def mock_extractor():
    return MockExtractor(id="mock_extractor", type="mock")


@pytest.fixture(scope="session")
def mock_loader():
    return MockLoad(id="mock_loader", type="mock")


@pytest.fixture(scope="session")
def mock_transformer_etl():
    return MockTransformETL(id="mock_transformer_etl")


@pytest.fixture(scope="session")
def mock_transformer_elt():
    return MockTransformELT(id="mock_transformer_elt", query="SELECT 1 FROM TABLE")
...
def test_run_transformer_elt(mock_transformer_elt) -> None:
    ...
    assert result == tf.TransformResult(
        name="mock_transformer_elt", type="ELT", success=True, error=None
    )
...
def test_etl_pipeline_init(etl_pipeline) -> None:
    ...
    assert len([etl_pipeline.extract.steps]) == 1 and isinstance(
        etl_pipeline.extract.steps[0], extract.IExtractor
    )
    assert len(etl_pipeline.transform.steps) == 1 and isinstance(
        etl_pipeline.transform.steps[0], tf.ITransformerETL
    )
    assert len(etl_pipeline.load.steps) == 1 and isinstance(
        etl_pipeline.load.steps[0], load.ILoader
    )
...
def test_elt_pipeline_init(elt_pipeline) -> None:
    ...
    assert len([elt_pipeline.extract.steps]) == 1 and isinstance(
        elt_pipeline.extract.steps[0], extract.IExtractor
    )
    assert len(elt_pipeline.transform.steps) == 1 and isinstance(
        elt_pipeline.transform.steps[0], tf.ITransformerELT
    )
    assert len(elt_pipeline.load.steps) == 1 and isinstance(
